...
class ContactForm {
  ...
  validationPassed () {
    ...
    $.ajax({
      ...
      error: function (error) {
        let dialogHtml = dialog.getHtml(
          'Something went wrong, please try to reach me in another way.'
        );
        ...
        ga(
          ...
          'Completed',
          'Email failed',
          reciverEmail.value + 'error:' + error
        );
      }
    });
  }
  ...
  clearErrors () {
    let unhappies = Array.from(
      document.getElementsByClassName('c-contact-form__error')
    );
